    ...
    return (
      <div>
        <h2 id="service-heading">
          <Translate contentKey="sistemaAmigoApp.service.home.title">Services</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sistemaAmigoApp.service.home.createLabel">Create a new Service</Translate>
          </Link>
        </h2>
        <Row>
          <Col sm="12">
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('sistemaAmigoApp.service.home.search')}
                  />
        ...
        <div className="table-responsive">
          {serviceList && serviceList.length > 0 ? (
              ...
              <tbody>
                {serviceList.map((service, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      <Translate contentKey={`sistemaAmigoApp.ServiceType.${service.type}`} />
                    </td>
                    <td>
                      <Translate contentKey={`sistemaAmigoApp.ServiceUnitType.${service.unit}`} />
                    </td>
                    <td>
                      <Translate contentKey={`sistemaAmigoApp.StatusType.${service.status}`} />
                    </td>
                    <td>{service.company ? <Link to={`company/${service.company.id}`}>{service.company.legalName}</Link> : ''}</td>
            ...
            <div className="alert alert-warning">
              <Translate contentKey="sistemaAmigoApp.service.home.notFound">No Services found</Translate>
            </div>
          )}
        </div>
      </div>
