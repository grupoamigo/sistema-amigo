...
import { getSearchEntities, getEntities } from './service-request.reducer';
...
export class ServiceRequest extends React.Component<IServiceRequestProps, IServiceRequestState> {
  state: IServiceRequestState = {
    search: ''
  };
    ...
    return (
      <div>
        <h2 id="service-request-heading">
          <Translate contentKey="sistemaAmigoApp.serviceRequest.home.title">Service Requests</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sistemaAmigoApp.serviceRequest.home.createLabel">Create a new Service Request</Translate>
          </Link>
        </h2>
        <Row>
          <Col sm="12">
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('sistemaAmigoApp.serviceRequest.home.search')}
                  />
        ...
        <div className="table-responsive">
          {serviceRequestList && serviceRequestList.length > 0 ? (
            <Table responsive aria-describedby="service-request-heading">
              <thead>
                <tr>
                  <th>
              ...
              <tbody>
                {serviceRequestList.map((serviceRequest, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      <Translate contentKey={`sistemaAmigoApp.StatusType.${serviceRequest.status}`} />
                    </td>
                    <td>
                      {serviceRequest.client ? <Link to={`client/${serviceRequest.client.id}`}>{serviceRequest.client.uniqueId}</Link> : ''}
                    </td>
                    <td>
            ...
            <div className="alert alert-warning">
              <Translate contentKey="sistemaAmigoApp.serviceRequest.home.notFound">No Service Requests found</Translate>
            </div>
          )}
        </div>
      </div>
