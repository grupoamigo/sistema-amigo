...
/**
 * REST controller for managing {@link com.sistema.amigo.domain.Client}.
 */
@RestController
@RequestMapping("/api")
public class ClientResource {
    ...
    public List<Client> getAllClients(@RequestParam(required = false) String filter) {
        if ("servicerequest-is-null".equals(filter)) {
            log.debug("REST request to get all Clients where serviceRequest is null");
            return StreamSupport
                .stream(clientRepository.findAll().spliterator(), false)
                .filter(client -> client.getServiceRequest() == null)
